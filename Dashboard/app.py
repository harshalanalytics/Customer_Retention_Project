...
geo = pd.crosstab(filtered_df["Geography"], filtered_df["Exited"], normalize="index") * 100
if 1 in geo.columns:
    st.bar_chart(geo[1])

st.subheader("Active vs Inactive Customer Retention")
active = pd.crosstab(filtered_df["IsActiveMember"], filtered_df["Exited"], normalize="index") * 100
if 1 in active.columns:
    st.bar_chart(active[1])

st.subheader("Gender-wise Churn Analysis")
gender = pd.crosstab(filtered_df["Gender"], filtered_df["Exited"], normalize="index") * 100
if 1 in gender.columns:
    st.bar_chart(gender[1])

st.subheader("Product Utilization Impact Analysis")
products = pd.crosstab(filtered_df["NumOfProducts"], filtered_df["Exited"], normalize="index") * 100
...
st.write(f"Number of disengaged high-value customers: {len(high_value_customers)}")

st.dataframe(
    high_value_customers[["CustomerId", "Balance", "EstimatedSalary", "IsActiveMember", "Exited"]].head(10)
)
